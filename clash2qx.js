...
if (!BODY) {
    ...
    //     // Clash: server, port, cipher, password, udp, name, plugin-opts, tls, sni
    //     // QX:    shadowsocks=host:port, method=cipher, password=pwd, obfs=..., tag=name
    //     return [];
    // };

    // handlers.vmess = function (raw) {
    //     // Clash: server, port, uuid, alterId, cipher, udp, tls, sni, network, ws-opts
    //     // QX:    vmess=host:port, method=none|aes-128-gcm, password=uuid, obfs=ws, tag=name
    //     return [];
    // };

    // handlers.vless = function (raw) {
    //     // Clash: server, port, uuid, udp, tls, sni, flow, network, ws-opts, reality-opts
    //     // QX:    vless=host:port, method=none, password=uuid, obfs=over-tls, tag=name
    //     return [];
    // };

    // handlers.trojan = function (raw) {
    //     // Clash: server, port, password, udp, sni, skip-cert-verify, network, ws-opts
    //     // QX:    trojan=host:port, password=pwd, over-tls=true, tls-host=..., tag=name
    //     return [];
    // };
    ...
    if (out.length === 0) {
        $done({ error: 'no supported nodes found in subscription' });
    } else {
